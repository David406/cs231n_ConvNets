...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        if self.cell_type == 'rnn':
            dword_vec, dh0, dWx, dWh, db = rnn_backward(dout, rnn_cache)
        if self.cell_type == 'lstm':
            dword_vec, dh0, dWx, dWh, db = lstm_backward(dout, lstm_cache)
        dW_embed = word_embedding_backward(dword_vec, embed_cache)
    ...
    def sample(self, features, max_length=30):
        ...
        x_1 = self._start * np.ones(N, dtype=int)
        embed_1 = W_embed[x_1, :]
        for i in range(max_length):
            if self.cell_type == 'rnn':
                h1, _ = rnn_step_forward(embed_1, h0, Wx, Wh, b)
                #h1 = embed_1.dot(Wx) + h0.dot(Wh) + b
            if self.cell_type == 'lstm':
                h1, c1, _ = lstm_step_forward(embed_1, h0, c0, Wx, Wh, b)
            scores = h1.dot(W_vocab) + b_vocab
            x_1 = np.argmax(scores, axis=1)
            captions[:, i] = x_1
            embed_1 = W_embed[x_1, :]
            h0 = h1
            if self.cell_type == 'lstm':
