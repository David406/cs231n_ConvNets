...
def rnn_backward(dh, cache):
    ...
    for t in reversed(range(T)):
        dxt, dprev_h, dWx_t, dWh_t, db_t = rnn_step_backward(dh[:,t,:] + dprev_h, cache[t])
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    doa = do * o * (1-o)
    dga = dg * (1-g**2)
    da = np.concatenate((dia, dfa, doa, dga), axis=1)
...
def lstm_backward(dh, cache):
    ...
    for t in reversed(range(T)):
        dxt, dprev_h, dnext_c, dWxt, dWht, dbt = lstm_step_backward(dh[:,t,:]+dprev_h, dnext_c, cache[t])
