...
def affine_backward(dout, cache):
    ...
    db = np.sum(np.where(dout != 0, dout, 0), axis=0)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx, dw, db
...
def batchnorm_backward(dout, cache):
    ...
    x_shifted = x - mean
    x_bar = (x-mean)/np.sqrt(var+eps)
    dgamma = np.sum(dout*x_bar, axis=0) 
    dbeta = np.sum(dout, axis=0)
    
    dout = dout * gamma
    dout_1 = dout / np.sqrt(var+eps)
    dout_2 = np.sum(x_shifted * dout, axis=0)
    #dout_2 = -1 * dout_2 / (2 * (var+eps)**1.5)
    #dout_2 = (2/N) * dout_2 * x_shifted
    dout_2 = -(1/N) * dout_2 * x_shifted / (var+eps)**1.5
    dout = dout_1 + dout_2
    
    dx = dout - (1/N) * np.sum(dout, axis=0)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def batchnorm_backward_alt(dout, cache):
    ...
    dx = (1/N) * gamma * (N * dout - dgamma * x_bar - dbeta) / np.sqrt(var+eps)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
        ...
        for f in range(F):
            for j in range(H_out):
                for k in range(W_out):
                    x_pad_slice = x_pad[i, :, j*stride: j*stride+HH, k*stride: k*stride+WW]
                    out[i, f, j, k] = np.sum(x_pad_slice * w[f])+b[f]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, w, b, conv_param)
        ...
        for f in range(F):
            for j in range(H_out):
                for k in range(W_out):
                    ...
                    dw[f] += x_pad[i, :, j*stride: j*stride+HH, k*stride: k*stride+WW] * dout[i,f,j,k]
                    db[f] += dout[i,f,j,k]
...
def max_pool_backward_naive(dout, cache):
    ...
    for i in range(N):
        for c in range(C):
            for j in range(H_out):
                for k in range(W_out):
                    x_slice = x[i, c, j*stride: j*stride+pool_height, k*stride: k*stride+pool_width]
                    s = (x_slice == np.max(x_slice))
                    dx[i, c, j*stride: j*stride+pool_height, k*stride: k*stride+pool_width] += s * dout[i,c,j,k]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx
